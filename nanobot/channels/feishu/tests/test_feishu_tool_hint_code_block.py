...
@pytest.mark.parametrize(
    "content, first_hint, second_hint",
    [
        pytest.param(
            "read src/main.py, grep \"TODO\"",
            "read src/main.py",
            "grep \"TODO\"",
            id="new_format_basic",
        ),
        pytest.param(
            "grep \"hello, world\", $ echo test",
            "grep \"hello, world\"",
            "$ echo test",
            id="new_format_with_comma_in_quotes",
        ),
        pytest.param(
            "read path × 3, grep \"pattern\"",
            "× 3",
            "grep \"pattern\"",
            id="new_format_with_folding",
        ),
        pytest.param(
            "web_search(\"foo, bar\"), read_file(\"/path/to/file\")",
            "web_search(\"foo, bar\")",
            "read_file(\"/path/to/file\")",
            id="keeps_commas_inside_arguments",
        ),
    ],
)
async def test_tool_hint_preserves_arguments(mock_feishu_channel, content, first_hint, second_hint):
    msg = OutboundMessage(
        channel="feishu",
        chat_id="oc_123456",
        content=content,
    ...
    with patch.object(mock_feishu_channel, '_send_message_sync') as mock_send:
        ...
        card = _get_tool_hint_card(mock_send)
        md = card["elements"][0]["content"]
        assert first_hint in md
        assert second_hint in md
